...
/***/ function(module, exports, __webpack_require__) {
	...
	var BoardWrapper = React.createClass({displayName: "BoardWrapper",
	handleNewGame: function () {
	      console.log('new game nigga');
	      this.forceUpdate()
	},
	    render: function () {
	        var game_ = Object.create(this.props.g);
	        game_.board = [3, 3, 3, 3, 3, 3, 3, 3, 3];
	        return (
	            React.createElement("div", {className: "wrap"}, 
	            React.createElement("button", {onClick: this.handleNewGame}, "New game"), 
	            React.createElement(Board, {newGame: this.handleNewGame, game: game_})
	...
	var Board = React.createClass({displayName: "Board",
	    ...
	    handleThePick: function (pick) {
	        console.log(pick);
	        var move = this.props.game.move(pick);

	        if (move && this.props.game.state > 3) {
	            this.props.game.calculateBestMove();
	        }

	        this.setState({tableState: this.props.game.board})

	    },
	    getInitialState: function () {
	        return {tableState:this.props.game.board}
	    },
	    componentWillReceiveProps: function (nextProps) {
	        this.setState({tableState: nextProps.game.board})
	    },
	    render: function () {
	        ...
	        var allSqaures = [];
	        this.state.tableState.forEach(function (s, index) {
	            var cName = function () {
	                if (index <= 3) {
	                    return 'top'
	                }
	            ...
	            allSqaures.push(React.createElement(Square, {marker: s, pleaseHandleMe: this.handleThePick, id: index +1, class: cName(), key: index}))
	        }.bind(this));

	        console.log('iam rending new stuff');
	        console.log(this.state.tableState);
	        console.log(this.props.game.board);
	        return (
	            React.createElement("div", {className: "wrap"}, 
	                React.createElement("h3", null, gameState), 
	...
	var Square = React.createClass({displayName: "Square",
	    render: function () {
	        ...
	        else if (this.props.marker == 0) {
	            marker = 'O'
	        }
	        return (
	            React.createElement("div", {onClick: this.pick, id: this.props.id, className: this.props.class}, marker)
	        )
	    },
	    pick: function (e) {
	    // call the callback fnc
	        this.props.pleaseHandleMe(e.target.id)
	    }


	});

	React.render(React.createElement(BoardWrapper, {g: game}), document.getElementById('c'));
...
/***/ function(module, exports, __webpack_require__) {
	...
	var game = {
	// 0 - O, 1 - X
	state: 4, // 4 - progress, 3 - tie, 0 - O won, 1 - X - won
	board: [],
	next: 1,
	...
	move: function (number) {
	    if (number < 1 || number > 9 || this.board[number - 1] !== 3) {
	        console.log(number + ' is not empty, select other move');
	        return false
	    }
	    this.board[number - 1] = this.next;
	    console.log(this.next + ' has moved to ' + number);
	...
	getIndexes: function () {
	    ...
	    for (var i = 0; i < 9; ++i) {
	        if (b[i] == 1) X.push(i +1);
	        else if (b[i] == 0) O.push(i +1);
	        else empty.push(i +1)
	    }
	    return {0: O, 1: X, empty: empty}
	...
	calculateBestMove: function () {
	    ...
	    var mover = this.next;
	    var enemy = this.next == 1 ? 0 : 1;

	    // if starting first
	    if (this.getIndexes().empty.length == 9) {
	        this.move(5);
	        return
	    }
	    var myWinMoves = this.getWinMoves(mover);
	    var enemyWinMoves = this.getWinMoves(enemy);

	    if (myWinMoves.winMoves.length == 0 && enemyWinMoves.winMoves.length == 0) {
	        console.log('DRAW!!');
	        this.state = 3;
	        return
	    }
	    if (myWinMoves.shortest == 1) {
	        var t = _.sortBy(myWinMoves.winMoves, function (n) {
	            return n.length
	        })[0];
	        this.move(t.pop());
	        return
	    }
	    if (enemyWinMoves.shortest == 1) {
	    console.log('enemy only 1 left to win');
	        var t = _.sortBy(enemyWinMoves.winMoves, function (n) {
	            return n.length
	        })[0];
	        this.move(t.pop());
	        return
	    }
	    if (myWinMoves.shortest == 3) {
	        if (this.isMiddleEmpty()) {
	            this.move(5)
	        }
	        else {
	            this.move(enemyWinMoves.winMoves.pop().pop())
	        }
	        return
	    }
	    if (myWinMoves.shortest == 2) {
	        if (this.isMiddleEmpty() && _.includes(_.flatten(myWinMoves.winMoves), 5)) {
	            this.move(5)
	        }
	        else {
	            this.move(_.flatten(myWinMoves.winMoves).pop())
	        }
	        return
	    }
	...
	getWinMoves: function (icon) {
	    var indexes = this.getIndexes();
	    var winMoves = [];
	    var shortest = 3;
	    for (var i = 0; i < this.winConditions.length; i++) {
	        var diff = _.difference(this.winConditions[i], indexes[icon]);
	        var free = _.difference(diff, indexes.empty);
	        if (diff.length <= 2 && free.length == 0) {
	            winMoves.push(diff);
	            if (diff.length < shortest) {
	                shortest = diff.length
	            }
	        }
	    }
	    return {winMoves: winMoves, shortest: shortest}
	},

	isMiddleEmpty: function () {
	    return this.board[4] === 3
	},

	_hasWon: function (icon) {
	    var indexes = this.getIndexes();
	    for (var i = 0; i < this.winConditions.length; i++){
	        var diff = _.difference(this.winConditions[i], indexes[icon]);
	        if (diff.length == 0) {
	            console.log((icon == 1? 'X' : 'O') + ' has won !!!');
	            this.state = icon == 1? 1 : 0
	        }
...
/***/ function(module, exports, __webpack_require__) {
	...
	if(false) {
		...
		module.hot.accept("!!B:\\Projects\\JavaScript\\react\\butkevicius.me\\node_modules\\css-loader\\index.js!B:\\Projects\\JavaScript\\react\\butkevicius.me\\public\\stylesheets\\thegame.css", function() {
			var newContent = require("!!B:\\Projects\\JavaScript\\react\\butkevicius.me\\node_modules\\css-loader\\index.js!B:\\Projects\\JavaScript\\react\\butkevicius.me\\public\\stylesheets\\thegame.css");
